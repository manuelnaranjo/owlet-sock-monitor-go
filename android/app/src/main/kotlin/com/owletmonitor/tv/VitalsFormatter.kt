...
object VitalsFormatter {
    ...
    fun sleepStateText(ctx: Context, raw: Int): String = when {
        ...
        raw >  2  -> ctx.getString(R.string.sleep_light)
    ...
    fun awakeDurationText(ctx: Context, startedAtMs: Long): String {
        val mins = ((System.currentTimeMillis() - startedAtMs) / 60_000).toInt()
        return if (mins < 60) ctx.getString(R.string.awake_for_minutes, mins)
               else ctx.getString(R.string.awake_for_hours, mins / 60, mins % 60)
    }
}
